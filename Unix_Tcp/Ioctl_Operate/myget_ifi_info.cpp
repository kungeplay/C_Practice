#include <string.h>
#include <netinet/in.h>//struct in_addr,struct sockaddr_in,struct in6_addr,struct sockaddr_in6
...
struct ifi_info * get_ifi_info(int family,int doaliases)
{
	...
	for(ptr=buf;ptr<buf+ifc.ifc_len;)
	{
		...
		ptr+=sizeof(ifr->ifr_name)+len;		//for the next one in buffer

#ifdef HAVE_SOCKADDR_DL_STRUCT
		//assumes that AF_LINK precedes AF_INET or AF_INET6
		if(ifr->ifr_addr.sa_family==AF_LINK)
		{
			struct sockaddr_dl *sdl=(struct sockaddr_dl *)&ifir->ifr_addr;
